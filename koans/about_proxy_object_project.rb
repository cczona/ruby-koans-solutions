...
class Proxy
  ...
  def called?(message)
  	@messages.include? message.to_sym
  end
  

  def number_of_times_called(message)
  	@messages.count message.to_sym
  end
  
  def method_missing(message, *args, &block)
		log_message(message)
		if @object.respond_to? message
			#puts "+++++++ #{@object} responded to #{message}"
			@object.send(message, *args, &block)   	
		else
			#puts "+++++++ #{@object} did not respond to #{message}. Its parent, #{super.class}, response is: #{super.respond_to? message}"
			super.send(message, *args, &block)
		end
	end
