...
module EdgeCase
  ...
  class Sensei
    ...
    def observe(step)
      if step.passed?
        ...
        if @pass_count > progress.last.to_i
          #CCZ#@observations << Color.green("#{step.koan_file}##{step.name} has expanded your awareness.")
        end
      else
        @failed_test = step
        @failure = step.failure
        add_progress(@pass_count)
        #CCZ#@observations << Color.red("#{step.koan_file}##{step.name} has damaged your karma.")
        throw :edgecase_exit
      end
    end
    ...
    def boring_end_screen
      #CCZ#puts "Mountains are again merely mountains"
    end
