...
class AboutMessagePassing < EdgeCase::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    #CCZ: so that send is available to be re-defined for other uses by objects, if desired i.e. email.send.  Re-defining either is allowed, but only __send__ will issue a warn() about it being a bad idea.
  end
  ...
  def test_catching_messages_makes_respond_to_untrustworthy
    catcher = AllMessageCatcher.new
  ...
  ##CCZ: collections and ranges
  def test_collection_indexing_boundaries_work_like_triple_dot_ranges
  	# think of these operations as specifying the surrounding fence lines, i.e. "everything inside fencepost zero and fencepost 3"
  	# 0 1 2 3 4 5 6 = fencepost locations
  	# | | | | | | | = fenceposts
  	#  a b c d e f  = example contents
  	# --------------
  	#  0 1 2 3 4 5  = double-dot style, i.e how PHP, Perl , and JavaScript index
  	  
  	a='abcdef'
  	b=[1,2,3,4,5,6]

  	assert_equal 'abc', a[0,3]
  	assert_equal 'cdef', a[2,6]
  	
  	assert_equal [1,2,3], b[0,3]
  	assert_equal [3,4,5,6], b[2,6]
  	
  	assert_equal 'abc', a[0...3]
  	assert_equal 'abc', a[0..2] # notice the double-dot
  ...
  def test_explicitly_implementing_respond_to_lets_objects_tell_the_truth
    ...
    # Consider making a factory that ensures that respond_to? is updated as part of any method_missing redefinition
  end
